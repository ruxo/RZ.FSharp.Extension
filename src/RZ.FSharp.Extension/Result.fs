...
let inline map (f: 'A -> 'B) (my: ValueResult<'A,'E>) :ValueResult<'B,'E> =
    match my with
...
let inline unwrapOrFail (messenger: 'err -> string) (x: ValueResult<'a, 'err>) :'a =
    match x with
    | ValueOk v -> v
    | ValueError e -> failwith <| messenger e

let inline unwrapOrRaise (thrower: 'err -> exn) (x: ValueResult<'a, 'err>) :'a =
    match x with
    | ValueOk v -> v
...
let inline filter (predicate: 'a -> bool) (error: 'a -> 'err) (x: ValueResult<'a,'err>) :ValueResult<'a,'err> =
    match x with
